...
Monkeypatches StockClient.get_klines with an engineered synthetic breakout
scenario so the full stack (API, DB, dashboard) can be exercised
end-to-end while this sandbox's egress policy blocks real market-data
hosts (Yahoo Finance included). This script is never used in production
and must never be pointed at by the real app — it exists purely to prove
the wiring works.
"""
...
import app.data.stock_client as sc


def fake_get_klines(self, symbol, interval, limit=500):
    seed = abs(hash(symbol)) % 1000
    df = make_synthetic_ohlcv(n=max(limit, 300), start_price=100, trend_per_bar=0.0, volatility=0.3, seed=seed, freq="1D")
...
sc.StockClient.get_klines = fake_get_klines
